...
@pytest.fixture
def sample_pdf_with_content(request):
...
def test_create_toc_page(pdf_reader):
...
def test_create_toc_page_empty_list(pdf_reader):
...
def test_create_toc_page_special_characters():
...
def test_create_toc_page_long_filename():
...
def test_create_title_page_long_filename():
...
def test_create_title_page_special_characters():
...
def test_get_unique_filename_special_characters(temp_dir):
...
def test_get_unique_filename_no_extension(temp_dir):
...
def test_merge_pdfs(temp_dir, sample_pdf):
...
def test_merge_pdfs_content_preservation(temp_dir, sample_pdf_with_content, request):
    """Test that PDF content is preserved during merging."""
    # Create a PDF with specific content
    dest = temp_dir / "test.pdf"
    with open(sample_pdf_with_content, 'rb') as src, open(dest, 'wb') as dst:
        dst.write(src.read())
    
    # Get the content string from the parameter
    content = request.node.callspec.params['sample_pdf_with_content']
    
    # Merge PDFs
    merge_pdfs(temp_dir, "merged.pdf")
    
    # Check content in merged PDF
    reader = PdfReader(str(temp_dir / "merged.pdf"))
    # Content should be on page 3 (after TOC and title page)
    content_text = reader.pages[2].extract_text()
    assert content in content_text


def test_merge_pdfs_no_files(temp_dir):
    """Test PDF merging with no input files."""
    merge_pdfs(temp_dir, "merged.pdf")
    assert not (temp_dir / "merged.pdf").exists()


def test_merge_pdfs_single_file(temp_dir, sample_pdf):
    ...
    reader = PdfReader(str(merged_path))
    # Expected: TOC + Title page + Content page = 3 pages
    assert len(reader.pages) == 3
    
    # Check if TOC includes the filename (without extension)
    toc_text = reader.pages[0].extract_text()
    ...
    assert reader.outline[0]['/Title'] == pdf_name


def test_merge_pdfs_duplicate_filenames(temp_dir, sample_pdf):
...
def test_merge_pdfs_non_pdf_files(temp_dir, sample_pdf):
...
def test_page_number_calculation(temp_dir, multi_page_pdf):
...
def test_toc_line_wrapping(pdf_reader):
...
def test_toc_dot_leaders(pdf_reader):
...
def test_toc_multiple_pages(pdf_reader):
...
def test_toc_formatting_consistency(pdf_reader):
